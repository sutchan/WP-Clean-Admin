...
function generate_po_content($entries, $header) {
    ...
    foreach ($entries as $msgid => $entry) {
        if ($msgid === '') continue; // 跳过空的 msgid（通常是头部）
...
function generate_mo_content($entries) {
    ...
    // 构建头部
    $mo_content = pack('L', $magic);
    $mo_content .= pack('L', $version);
    $mo_content .= pack('L', $num_strings);
    $mo_content .= pack('L', $offset_orig);
    $mo_content .= pack('L', $offset_trans);
    $mo_content .= pack('L', 0); // 哈希表大小（简化实现）
    $mo_content .= pack('L', $hash_table_offset);
