...
WP Clean Admin 已迭代至 1.8.2，存在以下问题需通过"原型 + 设计规范"统一后续开发：

1. **架构冗余**：旧式 `includes/class-wpca-*.php` 与模块化 `includes/modules/**/class-wpca-*.php` 功能重叠并存，缺乏统一设计约束，新功能易再次产生双份实现。
2. **前端缺乏统一规范**：各 AJAX handler 的 nonce 字段名曾出现 `_wpnonce` / `nonce` 不统一（已在 1.8.2 修复），本质是没有前端交互设计规范。
...
- 不修改现有 1.8.2 功能代码，仅新增 `assets/prototype/` 与文档
